"""
多项式回归适用于非线性关系的数据，输出为连续的预测值，通过拟合多项式曲线逼近目标值。
其优化目标是最小化均方误差（MSE），利用梯度下降法调整参数。
模型对噪声和异常值的鲁棒性较差，高阶特征可能放大噪声影响。
在收敛性上，合理设置学习率和条件可确保收敛，但计算复杂度较高，尤其高阶多项式会显著增加计算成本和存储需求。
扩展性方面，可通过增加高阶特征捕捉复杂关系，但需正则化避免过拟合。多项式回归适用于非线性回归任务，如曲线趋势预测或非线性过程建模。
"""
