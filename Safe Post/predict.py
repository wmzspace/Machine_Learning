...
def format_predictions(predictions, labels, threshold=0.5):
    ...
    for probs in predictions:
        # 将标签和概率配对，并按概率从高到低排序
        sorted_results = sorted([(LABELS[label], prob) for label, prob in zip(labels, probs)], key=lambda x: x[1],
                                reverse=True)
        # 筛选出概率超过阈值的标签
...
def main():
    ...
    parser.add_argument('--model', type=str, default="./model/model.joblib",
                        help="Path to the saved model file (default: ./model/model.joblib).")
    parser.add_argument('--vectorizer', type=str, default="./model/vectorizer.joblib",
                        help="Path to the saved vectorizer file (default: ./model/vectorizer.joblib).")
    parser.add_argument('--scaler', type=str, default="./model/scaler.joblib",
                        help="Path to the saved scaler file (default: ./model/scaler.joblib).")
    parser.add_argument('--data', type=str, nargs='+', required=True,
                        help="New text data for prediction. Pass one or more strings.")
    parser.add_argument('--threshold', type=float, default=0.5,
                        help="Probability threshold for displaying predictions (default: 0.5).")
    ...
    # 加载模型并预测
    predictions = load_model_and_predict(model_filename, vectorizer_filename, scaler_filename, processed_texts)
